...
public class BasicServerIntegrationTest {
    ...
    public void runTest() throws InterruptedException, IOException,
        ...
        LOG.debug("Running tests...");
         ...
         Set<Connection> connectionSet = ImmutableSet.of(new
         Connection(hostname, port));
         ImmutableMap<String, GearmanFunction> functions =
         ImmutableMap.of("reverse", (GearmanFunction)reverseFunc);
         WorkerRunner workerRunner = new WorkerRunner(connectionSet,
         functions);
         Thread t3 = new Thread(workerRunner);
         t3.start();

        String[] jobStrings = { "HELLO", "FOO", "BAR", "ONE", "BYE", "SNARF",
                "BAZ", "GEARMAN", "FOOBAR", "BIGFOOT" };
...
//         assertEquals(integer.get(), 100);
    ...
    public static void main(String... args) throws InterruptedException,
            IOException, JobSubmissionException, WorkException {
    ...
    private class CountingReverseFunc implements GearmanFunction {
        ...
        public byte[] process(WorkEvent workEvent) {
            ...
            hits += 1;
            LOG.debug(String.format("Processing job '%s' with data '%s'",
                    job.getFunctionName(), new String(job.getData())));
