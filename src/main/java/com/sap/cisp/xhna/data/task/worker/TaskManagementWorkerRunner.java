 ...
 * 
 * The worker polls jobs from a job server and execute the function.
 * This gearman worker API is aligned with the gearman server version.
 * As an alternative embedded worker solution for worker pool.
 * 
 */
public class TaskManagementWorkerRunner extends WorkerRunner implements IWorker {
    ...
    public static Logger logger = LoggerFactory
            .getLogger(TaskManagementWorkerRunner.class);
    ...
    public static TaskManagementWorkerRunner getInstance() {
        return TaskManagementWorkerHolder.helper;
    }

    private static class TaskManagementWorkerHolder {
        public static TaskManagementWorkerRunner helper = new TaskManagementWorkerRunner();
    }

    private TaskManagementWorkerRunner() {
        // private constructor
    }

    private TaskManagementWorkerRunner(Set<Connection> connections,
            Map<String, GearmanFunction> functions) {
        this.functions = functions;
        this.connections = connections;
    }

    public TaskManagementWorkerRunner init(
        ...
        Set<Connection> connectionSet = ImmutableSet.of(new Connection(
                GEARMAN_HOST, port));
        Map<String, GearmanFunction> functions = new HashMap<String, GearmanFunction>();
        functions.put(TaskManagementUtils.FunctionEnum.FIND_SOCIAL_ACCOUNT_BY_KEYWORD
                        .toString(), this);
                        functions.put(TaskManagementUtils.FunctionEnum.VERIFY_SOCIAL_ACCOUNT
                        .toString(), this);
        ...
        if (ConfigInstance.getCurrentWorkMode() != TaskManager.WorkMode.WORKER_ONLY) {
            functions.put(
                    ...
                    this);
            functions.put(TaskManagementUtils.FunctionEnum.RELEASE_TOKEN_TAG
                    .toString(), this);
            functions.put(TaskManagementUtils.FunctionEnum.CHECK_IF_UNCACHED_SOCIAL_POSTS
                            .toString(), this);
            functions.put(TaskManagementUtils.FunctionEnum.CHECK_TRADITIONAL_POST_CACHED
                            .toString(), this);
            functions.put(
                    TaskManagementUtils.FunctionEnum.GET_UNCACHED_SOCIAL_POSTS
                            .toString(), this);
        }
        functions.put(TaskManagementUtils.FunctionEnum.CANCEL_MULTIPLE_TASK
                .toString(), this);
        functions.put(
                TaskManagementUtils.FunctionEnum.CANCEL_SINGLE_TASK.toString(),
                this);

        functions.put(TaskManagementUtils.FunctionEnum.TEST_TASK.toString(),
                this);
        this.functions = functions;
    ...
    public byte[] process(WorkEvent workEvent) {
        synchronized (TaskManagementWorkerRunner.class) {
    ...
    private byte[] releaseToken(byte[] data) {
        synchronized (TaskManagementWorkerRunner.class) {
            ...
            logger.debug(
                    "Release token : tokenTag {}; statusCode {}; Has Exception {}",
