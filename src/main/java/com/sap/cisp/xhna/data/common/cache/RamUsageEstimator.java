 ...
 * <p>
 * This class uses assumptions that were discovered for the Hotspot virtual
 * machine. If you use a non-OpenJDK/Oracle-based JVM, the measurements may be
 * slightly wrong.
 * 
...
public final class RamUsageEstimator {
    ...
    static final boolean JRE_IS_64BIT = true; // Add as hardcode for now
    ...
    static {
        if (JRE_IS_64BIT) {
            // Try to get compressed oops and object alignment (the default
            // seems to be 8 on Hotspot);
            // (this only works on 64 bit, on 32 bits the alignment and
            ...
            try {
                ...
                if (hotSpotBean != null) {
                    ...
                    try {
                        final Object vmOption = getVMOptionMethod.invoke(
                                hotSpotBean, "ObjectAlignmentInBytes");
                        objectAlignment = Integer.parseInt(vmOption.getClass()
                                .getMethod("getValue").invoke(vmOption)
                                .toString());
                    } catch (ReflectiveOperationException | RuntimeException e) {
                        isHotspot = false;
                    }
            ...
            // array header is NUM_BYTES_OBJECT_HEADER + NUM_BYTES_INT, but
            // aligned (object alignment):
        ...
        while (longCacheMinValue > Long.MIN_VALUE
                && Long.valueOf(longCacheMinValue - 1) == Long
                        .valueOf(longCacheMinValue - 1)) {
            longCacheMinValue -= 1;
        }
        long longCacheMaxValue = -1;
        while (longCacheMaxValue < Long.MAX_VALUE
                && Long.valueOf(longCacheMaxValue + 1) == Long
                        .valueOf(longCacheMaxValue + 1)) {
            longCacheMaxValue += 1;
        }
    ...
    public static long shallowSizeOf(Object[] arr) {
        return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
                + (long) NUM_BYTES_OBJECT_REF * arr.length);
    }
